...
```mermaid
flowchart LR
  GoverRule["goverrule.Server"] --> Record["RecordHistory"]
  Record --> Composite["CompositeHistory"]
  Composite --> Logger["history/logger"]
  Composite --> RDS["history/rds"]
```
...
sequenceDiagram
  ...
  participant Handler as PluginInstanceEventHandler
  participant Channel as CompositeDiscoverChannel
  participant Entry as logger / rds

  Service->>EventHub: Publish InstanceEventTopic
  EventHub->>Handler: OnEvent
  Handler->>Channel: PublishEvent
  Channel->>Entry: PublishEvent
```
...
```mermaid
flowchart LR
  Discover["Discover hot path"] --> CacheMetric["reportDiscoverCacheCall"]
  CacheMetric --> Statis["statis.GetStatis"]
  Statis --> Local["statis/logger"]
  Statis --> Prom["statis/prometheus"]
```
...
```mermaid
flowchart TB
  Runtime["runtime / cache / batch / client"] --> Metrics["pkg/common/otel/metrics"]
  Metrics --> Meter["otel.Meter()"]
  Meter --> Exporter["OTLP metric exporter"]
  Trace["trace provider"] --> TraceExporter["OTLP trace exporter"]
  Exporter --> Backend["OTel Collector / OpenObserver / backend"]
  TraceExporter --> Backend
```
