...
```mermaid
flowchart TB
  RuleStore["规则表 / rule"] --> ConsoleCache["Console cache: ruleID -> rule"]
  ReleaseStore["发布表 / release"] --> ClientCache["Client cache: ActiveKey -> active release"]
  ConsoleCache --> Console["Console 列表 / 详情 / 编辑"]
  ClientCache --> SDK["SDK / xDS / Nacos / HTTP Discover"]
```
...
sequenceDiagram
  ...
  participant Auth as goverrule auth proxy
  participant Param as paramcheck proxy
  participant Rule as goverrule.Server
  participant Store as MySQL store
  participant History as history chain

  Console->>HTTP: Create / Update rule
  HTTP->>Auth: 调用 GoverRuleServer
  Auth->>Auth: CheckConsolePermission
  Auth->>Param: 通过后进入参数校验
  Param->>Rule: 调用真实 Server
  Rule->>Rule: 生成 ID / Revision
  Rule->>Store: CreateRule / UpdateRule
  Store-->>Rule: success
  Rule->>History: RecordHistory
  Rule-->>Console: BatchWriteResponse
```
...
```mermaid
flowchart TB
  Publish["PublishGovernanceRules"] --> Auth["collectRuleReleases + CheckConsolePermission"]
  Auth --> Tx["Store transaction"]
  Tx --> Inactive["旧 release active = 0"]
  Inactive --> Insert["写入新 release"]
  Insert --> Version["version = max + 1"]
  Version --> CachePoll["治理 cache 增量更新"]
  CachePoll --> ActiveView["Client active release 视图"]
```
